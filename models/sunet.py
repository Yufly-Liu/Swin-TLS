"""SUNet - Swin Transformer U-Net for Image Denoising.

原版 SUNet 实现，来自 SUNet-main 项目。
使用 Dual up-sample (PixelShuffle + Bilinear) 上采样策略。
包装为 BaseModel 接口以适配训练框架。
...
# ============================================================
# 原版 SUNet 组件（来自 SUNet-main/model/SUNet_detail.py）
# ============================================================
...
class UpSample(nn.Module):
    """Dual up-sample: PixelShuffle + Bilinear"""
...
class _SUNetCore(nn.Module):
    """原版 SUNet 核心网络（不继承 BaseModel，纯网络结构）"""
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, out_chans=3,
        ...
        if self.final_upsample == "Dual up-sample":
            ...
            self.output = nn.Conv2d(embed_dim, self.out_chans, kernel_size=3, stride=1, padding=1, bias=False)
...
class SUNet(BaseModel):
    """SUNet - 原版 Swin Transformer U-Net 去噪网络。

    使用原版 SUNet-main 的完整架构：
    - 4 层编码器 + 4 层解码器
    - Dual up-sample (PixelShuffle + Bilinear)
    ...
    Config keys:
        ...
        img_size (int): 输入图像尺寸，默认 256（必须是 patch_size * window_size * 2^(num_layers-1) 的倍数）
        embed_dim (int): 嵌入维度，默认 96
        depths (list): 每级 Transformer 块数，默认 [8, 8, 8, 8]
        num_heads (list): 每级注意力头数，默认 [8, 8, 8, 8]
        window_size (int): 窗口大小，默认 8
        patch_size (int): Patch 大小，默认 4
        mlp_ratio (float): MLP 隐藏层比例，默认 4.0
        drop_rate (float): Dropout 率，默认 0.0
        drop_path_rate (float): DropPath 率，默认 0.1
        qk_scale (int): QK 缩放因子，默认 8
    """
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.net = _SUNetCore(
            ...
            depths=self.config.get("depths", [8, 8, 8, 8]),
            num_heads=self.config.get("sunet_num_heads", self.config.get("num_heads", [8, 8, 8, 8])),
            window_size=self.config.get("window_size", 8),
            mlp_ratio=self.config.get("mlp_ratio", 4.0),
            qkv_bias=self.config.get("qkv_bias", True),
            qk_scale=self.config.get("qk_scale", 8),
            drop_rate=self.config.get("drop_rate", 0.0),
            attn_drop_rate=self.config.get("attn_drop_rate", 0.0),
            drop_path_rate=self.config.get("drop_path_rate", 0.1),
            ape=self.config.get("ape", False),
            patch_norm=self.config.get("patch_norm", True),
            use_checkpoint=self.config.get("use_checkpoint", False),
            final_upsample=self.config.get("final_upsample", "Dual up-sample"),
        )
