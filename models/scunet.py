"""SCUNet - Structured Crowd U-Net for Image Denoising.

From SCUNet-main project.
Wrapper to adapt to BaseModel interface for training framework.
...
class _SCUNetCore(nn.Module):
    """SCUNet 核心网络（不继承 BaseModel，纯网络结构）"""

    def __init__(self, in_nc=3, config=[2, 2, 2, 2, 2, 2, 2], dim=64, drop_path_rate=0.0, input_resolution=256):
        super(_SCUNetCore, self).__init__()
        self.config = config
        self.dim = dim
        self.head_dim = 32
        self.window_size = 8
...
class SCUNet(BaseModel):
    """SCUNet - Structured Crowd U-Net 去噪网络。

    使用 Swin Transformer block 和卷积 block 的混合架构。

    Config keys:
        in_channels (int): 输入通道数，默认 3
        out_channels (int): 输出通道数，默认 3
        dim (int): 基础维度，默认 64
        config (list): 每层 block 数量，默认 [2,2,2,2,2,2,2]
        drop_path_rate (float): DropPath 率，默认 0.0
        input_resolution (int): 输入分辨率，默认 256
    """

    def __init__(self, config: Dict[str, Any] = None):
        super().__init__(config)

        self.net = _SCUNetCore(
            in_nc=self.in_channels,
            config=self.config.get("scunet_config", [2, 2, 2, 2, 2, 2, 2]),
            dim=self.config.get("scunet_dim", 64),
            drop_path_rate=self.config.get("scunet_drop_path_rate", 0.0),
            input_resolution=self.config.get("scunet_input_resolution", 256),
        )
